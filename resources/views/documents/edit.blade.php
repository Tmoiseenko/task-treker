...
<div class="mb-6">
    <div class="flex items-center gap-2 mb-2">
        <a href="{{ route('documents.show', $document) }}" class="text-gray-500 hover:text-gray-700">
            ← Назад к документу
        </a>
    </div>
    <h1 class="text-3xl font-bold text-gray-900">Редактировать документ</h1>
    <p class="text-sm text-gray-500 mt-1">Текущая версия: {{ $document->version }}</p>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-lg shadow-sm p-6">
            <form method="POST" action="{{ route('documents.update', $document) }}" x-data="documentForm()">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                        ...
                        <select name="category" 
                            ...
                            @foreach(\App\Enums\DocumentCategory::cases() as $category)
                                <option value="{{ $category->value }}" 
                                    {{ old('category', $document->category?->value) == $category->value ? 'selected' : '' }}>
                                    {{ match($category) {
                    ...
                    <div>
                        ...
                        <div class="mb-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                            <p class="text-sm text-yellow-800">
                                <strong>Внимание:</strong> Изменение содержания создаст новую версию документа (версия {{ $document->version + 1 }})
                            </p>
                        </div>
                        ...
                        <div x-show="!showPreview">
                            <textarea name="content" 
                                      ...
                                      class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 font-mono text-sm @error('content') border-red-500 @enderror">{{ old('content', $document->content) }}</textarea>
                            @error('content')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('documents.show', $document) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold py-2 px-6 rounded-lg">
                            Отмена
                        </a>
                    </div>
                </div>
    ...
    <!-- Sidebar with Markdown Help and Version Info -->
    <div class="space-y-6">
        <!-- Version Info -->
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">Информация о версиях</h2>
            <dl class="space-y-3 text-sm">
                <div>
                    <dt class="font-medium text-gray-700">Текущая версия</dt>
                    <dd class="mt-1 text-gray-900">{{ $document->version }}</dd>
                </div>
                <div>
                    <dt class="font-medium text-gray-700">Всего версий</dt>
                    <dd class="mt-1 text-gray-900">{{ $document->versions->count() }}</dd>
                </div>
                <div>
                    <dt class="font-medium text-gray-700">Последнее изменение</dt>
                    <dd class="mt-1 text-gray-900">{{ $document->updated_at->format('d.m.Y H:i') }}</dd>
                </div>
            </dl>
        </div>
...
function documentForm() {
    return {
        content: @js(old('content', $document->content)),
        showPreview: false
    }
}
