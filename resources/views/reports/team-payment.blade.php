...
@section('title', 'Отчет по выплатам команде')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900 mb-2">Отчет по выплатам команде</h1>
    <p class="text-gray-600">Сводный отчет по всем специалистам</p>
</div>

<!-- Date Range Filter -->
<div class="bg-white rounded-lg shadow-sm p-6 mb-6">
    <form method="GET" action="{{ route('reports.team-payment') }}" class="space-y-4">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="text-sm font-medium text-gray-500 mb-1">Общая сумма выплат</div>
...
<div class="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Выплаты по специалистам</h2>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($report['by_user'] as $userData)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-gray-900">${{ number_format($userData['total_payment'], 2) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <a href="{{ route('reports.user-payment', ['user' => $userData['user_id'], 'from' => $from->format('Y-m-d'), 'to' => $to->format('Y-m-d')]) }}" 
...
<div class="bg-white rounded-lg shadow-sm overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Выплаты по проектам</h2>
    </div>
    ...
    @else
        <div class="p-12 text-center text-gray-500">
            <p class="text-lg">Проекты отсутствуют</p>
        </div>
    @endif
</div>
@endsection
